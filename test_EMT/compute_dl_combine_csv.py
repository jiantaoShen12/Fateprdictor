...
T1=1
T2=3
path_in=f"/home/sjt/workspace/beginning_project/cell_change尝试/test_EMT/output/{T}/LSTM/"
df = pd.read_csv(path_in+f'df_dl.csv')
time1 = df['time'].drop_duplicates()
time1 = time1.to_frame(name='time')
print(time1)
...
# Load models
    if  a == 1:
        ...
        path_in=f"/home/sjt/workspace/beginning_project/cell_change尝试/test_EMT/output/{T}/GRU/"
        df = pd.read_csv(path_in+f'df_dl.csv')
        df1=df[df['classifier'] == 'c1']
        df1.dropna(axis=0, how='any')
        df2=df[df['classifier'] == 'c2']
        ...
        path_in=f"/home/sjt/workspace/beginning_project/cell_change尝试/test_EMT/output/{T}/BILSTM/"
        df = pd.read_csv(path_in+f'df_dl.csv')
        df1=df[df['classifier'] == 'c1']
        df1.dropna(axis=0, how='any')
        df2=df[df['classifier'] == 'c2']
...
preds_prob11 = pd.DataFrame(preds_prob)
time1=pd.DataFrame(time1)
preds_prob11 = pd.concat([preds_prob11, time1], axis=1)
output_dir = f'/home/sjt/workspace/beginning_project/cell_change尝试/test_EMT/output/'
preds_prob11.to_csv(f"{output_dir}{T1}_{T2}dl.csv")
...
# 定义颜色列表，每个颜色对应一个轨迹
colors = ['royalblue', 'orangered', 'lime', 'magenta', 'orange', 'cyan']
...
fig.update_layout(
    ...
    xaxis_range=[0, 700],  # 设置x轴范围为完整数据集的范围
    showlegend=True,
...
# 设置图形的保存路径
file_path = f'/home/sjt/workspace/beginning_project/cell_change尝试/test_EMT/output/{T}/combined_pd.png'

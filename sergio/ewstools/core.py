...
class TimeSeries:
    ...
    transition : float
        ...
        `TimeSeries` 类在 `ewstools` 包中主要负责处理单变量时间序列数据，并计算早期预警信号（EWS）。以下是 `TimeSeries` 类的主要职责和功能：

1. **初始化时间序列数据**：通过构造函数 `__init__`，`TimeSeries` 类接收时间序列数据和一个可选的转换时间（transition），如果提供，EWS 仅计算到此时间点。

2. **数据存储**：时间序列数据和转换时间被存储为类的属性。数据通常以 Pandas DataFrame 的形式存储，其中索引代表时间。

3. **去趋势**：`detrend` 方法使用高斯核或LOWESS方法去除时间序列数据的趋势。

4. **计算EWS**：类中包含多个方法来计算不同的EWS，如：
   - `compute_var`：计算滚动窗口内的方差。
   - `compute_std`：计算滚动窗口内的标准差。
   - `compute_cv`：计算滚动窗口内的变异系数。
   - `compute_auto`：计算滚动窗口内的自相关。
   - `compute_skew`：计算滚动窗口内的偏度。
   - `compute_kurt`：计算滚动窗口内的峰度。
   - `compute_ktau`：计算Kendall tau值，用于量化趋势。

5. **深度学习分类器应用**：`apply_classifier` 和 `apply_classifier_inc` 方法允许将深度学习分类器应用于时间序列数据，以预测临界点。
6. **可视化**：`make_plotly` 方法生成一个交互式的 Plotly 图形，用于可视化计算的EWS。
7. **清除深度学习预测**：`clear_dl_preds` 方法清除深度学习预测结果。
8. **计算功率谱**：`compute_spectrum` 方法计算时间序列的功率谱。
9. **计算Smax**：`compute_smax` 方法计算功率谱的最大值。
10. **拟合功率谱类型**：`compute_spec_type` 方法拟合不同的功率谱类型，并计算AIC权重。
11. **辅助方法**：类还包含一些辅助方法，如 `compute_covar` 和 `compute_corr`，用于计算协方差矩阵和相关矩阵。
`TimeSeries` 类是 `ewstools` 中用于时间序列分析的核心组件，提供了一套完整的工具来处理和分析时间序列数据，以便研究和预测系统可能的临界转变。
    ...
    def detrend(self, method="Gaussian", bandwidth=0.2, span=0.1):#原来是0.2
        """
    ...
    def apply_classifier_inc(self, classifier, inc=10, name="c1", verbose=1):
        ...
        """
        if len(self.state) < 2:
            raise ValueError("The state data must contain at least two elements to compute dt.")
        else:

...
np.random.seed(0)
name2=['fold_forced_g1','fold_forced_g2','pf_forced','null']
i=3

path_in="/home/sjt/workspace/beginning_project/cell_change尝试/simple_gene_model/output/LSTM/"
df = pd.read_csv(path_in+f'df_{name2[i]}.csv')
...
# Load models
    if  a == 1:
        ...
        path_in="/home/sjt/workspace/beginning_project/cell_change尝试/simple_gene_model/output/GRU/"
        df = pd.read_csv(path_in+f'df_{name2[i]}.csv')
        df1=df[df['classifier'] == 'c1']
        df1.dropna(axis=0, how='any')
        df2=df[df['classifier'] == 'c2']
        ...
        path_in="/home/sjt/workspace/beginning_project/cell_change尝试/simple_gene_model/output/BILSTM/"
        df = pd.read_csv(path_in+f'df_{name2[i]}.csv')
        df1=df[df['classifier'] == 'c1']
        df1.dropna(axis=0, how='any')
        df2=df[df['classifier'] == 'c2']
...
fig.update_layout(
    ...
    xaxis_range=[0, 699],  # 设置x轴范围为完整数据集的范围
...
# 设置图形的保存路径
file_path = f'/home/sjt/workspace/beginning_project/cell_change尝试/simple_gene_model/output/combined_{name2[i]}.png'
# 保存图形到文件
